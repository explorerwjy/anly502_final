...
def get_business_list(num,category,city,business_data):
    ...
    counts = counts.map(lambda x: (x[0]["business_id"],x[0]["name"],x[0]["stars"],x[1])).take(n)
    city_name = "_".join(city.split())
    file_name = "best_"+str(n)+"_"+category+"_in_"+city_name+".txt"
    with open(file_name,'w') as fout:
        for (bus,name,city,,stars,count) in counts:
            fout.write("{}\t{}\t{}\t{}\n".format(bus,name,stars,count))
    return file_name
...
def get_data_according_to_list_file(bus_list_file,business_data,review_data,tip_data,user_data):
	...
	bus_list = [id_ for id_,name,start,count in handle.readline().split('\t')]
	print bus_list
	#filter business 
...
if __name__ == "__main__":
    ...
    # Procedures starts
    fileanme = get_business_list(100,"Restaurants","Las Vegas",business_data)
    #get_data_according_to_list_file(fileanme,business_data,review_data,tip_data,user_data)
